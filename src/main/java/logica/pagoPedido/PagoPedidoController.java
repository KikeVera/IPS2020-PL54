...
/**
 * Es el controlador de la ventana de pago de pedido , accesible 
...
public class PagoPedidoController implements Controller{
	...
	/**
	 * Comprueba que los datos de la tarjeta introducidos por el usuario son correctos y en caso de que 
	 * las validaciones salieran correctamente, habilita la opcion de finalizar pedido en la tienda online  
	 */
	private void checkTarjeta() {
		...
		if(view.getTextNumeroTarjeta().getText().length() != 16) {
			JOptionPane.showMessageDialog(this.view.getFrame(), "El número de la tarjeta debe estar compuesto por 16 dígitos",
					"Tienda online: Advertencia", JOptionPane.WARNING_MESSAGE);
			return; 
		}
		...
		if(view.getTextCVV().getText().length() != 3) {
			JOptionPane.showMessageDialog(this.view.getFrame(), "El CVV de la tarjeta debe estar compuesto por 3 dígitos",
					"Tienda online: Advertencia", JOptionPane.WARNING_MESSAGE);
			return; 
		}
